# DevOps Project: Балансировщик нагрузки HAProxy + Flask приложение

## 📋 Описание проекта
Проект автоматизации развертывания высокодоступного веб-приложения с использованием:
- **Terraform**: Инфраструктура как код (виртуальные машины, БД, группы безопасности)
- **Ansible**: Управление конфигурацией и развертывание приложений  
- **HAProxy**: Балансировка нагрузки с SSL терминацией
- **Flask**: Python веб-приложение с MySQL базой данных

## 🏗️ Архитектура системы
[Интернет] → [HAProxy Балансировщик] → [Flask App 1] → [MySQL База данных]
→
[Flask App 2]



## 📁 Структура проекта
```
project/
├── ansible/
│   ├── group_vars/all/ # Переменные (vars.yml, vault.yml)
│   ├── inventory/ # Инвентаризация (hosts.yml)
│   ├── roles/ # Роли Ansible
│   │   ├── app/ # Flask приложение
│   │   ├── haproxy/ # HAProxy + SSL
│   │   └── hardening/ # Безопасность ОС
│   └── playbook.yml # Основной плейбук
├── terraform/
│   ├── main.tf # Основная конфигурация
│   ├── database.tf # База данных
│   ├── variables.tf # Переменные
│   ├── outputs.tf # Выходные данные
│   └── vkcs_provider.tf # Провайдер VK Cloud
└── README.md # Документация
```
## 🚀 Быстрый старт

### 1. Развертывание инфраструктуры с Terraform
cd terraform

# Инициализация
terraform init

# Планирование
terraform plan \
  -var="username=ВАШ_EMAIL" \
  -var="password=ВАШ_ПАРОЛЬ" \
  -var="project_id=ВАШ_PROJECT_ID" \
  -var="lastname=ВАША_ФАМИЛИЯ" \
  -var="ssh_public_key=$(cat ~/.ssh/id_rsa.pub)"

# Применение
terraform apply [параметры как выше]
2. Получение IP адресов
После выполнения terraform apply вы получите выходные данные:

```
Outputs:

application_urls = {
  "app_servers" = [
    "http://192.168.100.249:5000",  # ПРИВАТНЫЙ IP app1
    "http://192.168.100.192:5000",  # ПРИВАТНЫЙ IP app2
  ]
  "haproxy_public" = "http://212.111.84.45"  # ПУБЛИЧНЫЙ IP haproxy
}

public_ips = {
  "app1" = "212.111.84.154"      # ПУБЛИЧНЫЙ IP app1
  "app2" = "212.111.86.216"      # ПУБЛИЧНЫЙ IP app2
  "haproxy" = "212.111.84.45"    # ПУБЛИЧНЫЙ IP haproxy
}
```

3. Настройка Ansible Inventory
Обновите ansible/inventory/hosts.yml с полученными IP:

```
all:
  vars:
    ansible_user: ubuntu
    ansible_ssh_private_key_file: ~/.ssh/id_rsa
    ansible_become: yes

app_servers:
  hosts:
    app1:
      ansible_host: "212.111.84.154"      # ПУБЛИЧНЫЙ IP app1
      private_ip: "192.168.100.249"       # ПРИВАТНЫЙ IP app1
    app2:
      ansible_host: "212.111.86.216"      # ПУБЛИЧНЫЙ IP app2
      private_ip: "192.168.100.192"       # ПРИВАТНЫЙ IP app2

haproxy:
  hosts:
    haproxy-server:
      ansible_host: "212.111.84.45"       # ПУБЛИЧНЫЙ IP haproxy
      private_ip: "192.168.100.193"       # ПРИВАТНЫЙ IP haproxy

database:
  hosts:
    db:
      ansible_host: "212.111.86.216"      # ПУБЛИЧНЫЙ IP Managed MySQL
```

4. Решение проблем с SSH подключением

cd ansible

# Очистка старых SSH ключей
```
ssh-keygen -f ~/.ssh/known_hosts -R '212.111.84.154'
ssh-keygen -f ~/.ssh/known_hosts -R '212.111.86.216'  
ssh-keygen -f ~/.ssh/known_hosts -R '212.111.84.45'
```
# Добавление новых ключей
```
ssh-keyscan 212.111.84.154 >> ~/.ssh/known_hosts
ssh-keyscan 212.111.86.216 >> ~/.ssh/known_hosts
ssh-keyscan 212.111.84.45 >> ~/.ssh/known_hosts
```
5. Запуск Ansible плейбука

cd ansible

# Запуск полного плейбука

ansible-playbook -i inventory/hosts.yml playbook.yml --ask-vault-pass

# Пароль vault: 1954
🔐 Система паролей
Пароль базы данных
Источник: Задается в Terraform (variables.tf)
```
hcl
variable "db_password" {
  description = "Database password"
  type        = string
  sensitive   = true
  default     = "7h78gs.p70aG85wU0"  # ← Пароль БД
}
```
Ansible Vault
Пароль: 1954 - для расшифровки секретов

Содержимое vault.yml:
```
db_password: "7h78gs.p70aG85wU0"  # Пароль MySQL (из variables.tf)
secret_key: "dddd"                # Секретный ключ приложения
Управление vault:
```
# Просмотр секретов
ansible-vault view group_vars/all/vault.yml

# Редактирование
ansible-vault edit group_vars/all/vault.yml

# Смена пароля
ansible-vault rekey group_vars/all/vault.yml

🧪 Тестирование
После развертывания проверьте работу системы:


# Проверка HTTP -> HTTPS редиректа
curl -I http://212.111.84.45

# Проверка HTTPS
curl -k https://212.111.84.45

# Проверка домена Let's Encrypt
curl -k https://212-111-84-45.nip.io

🛠️ Функциональность

✅ Создаваемая инфраструктура:

3 виртуальные машины в разных зонах доступности

Managed MySQL 8.0

Группы безопасности

✅ Настраиваемые компоненты:

HAProxy с балансировкой нагрузки (round-robin)

Let's Encrypt SSL сертификаты

Flask приложения на двух серверах

Безопасность: UFW, fail2ban, харденинг SSH

🔒 Безопасность

База данных доступна только с app-серверов

SSH доступ только по ключам

Файрвол с минимально открытыми портами

Шифрование секретов через Ansible Vault
